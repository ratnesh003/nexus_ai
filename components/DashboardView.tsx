...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip as ReTooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell, LineChart, Line, RadarChart, Radar, PolarGrid, PolarAngleAxis, PolarRadiusAxis, RadialBarChart, RadialBar } from 'recharts';
...
const DashboardView: React.FC<DashboardViewProps> = ({ projectId }) => {
    ...
    const renderChart = (config: ChartConfig) => {
        const CommonTooltip = () => <ReTooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />;
        ...
        switch (type) {
            ...
            case 'line':
                 return (
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={config.data} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                            <XAxis dataKey="name" fontSize={12} tickLine={false} axisLine={false} />
                            <YAxis fontSize={12} tickLine={false} axisLine={false} />
                            <CommonTooltip />
            ...
            case 'pie':
                return (
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie data={config.data} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                ...
                return (
                     <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={config.data}>
                            ...
                            <Radar name={config.title} dataKey="value" stroke="#82ca9d" fill="#82ca9d" fillOpacity={0.6} />
                            <CommonTooltip />
                        </RadarChart>
                    </ResponsiveContainer>
                );
            ...
            default:
                return <div className="flex items-center justify-center h-full text-xs text-muted-foreground">Unsupported chart type: {type}</div>;
        }
    };
    ...
    if (loading) {
        return (
            <div className="h-full flex flex-col items-center justify-center space-y-4">
                ...
                <p className="text-muted-foreground animate-pulse">AI is analyzing your data and generating visualizations...</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex h-full bg-slate-50">
            ...
            <div className="w-56 bg-white border-r border-border p-4 space-y-1 flex-shrink-0">
                <div className="text-xs font-semibold text-muted-foreground uppercase mb-3 px-2">Visualization Type</div>
                {['main', 'bar', 'pie', 'line', 'radar', 'radial'].map(type => (
                    <button
                        key={type}
                        onClick={() => setActiveTab(type)}
                        className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-colors flex justify-between items-center ${activeTab === type ? 'bg-accent/10 text-accent' : 'text-slate-600 hover:bg-slate-100'}`}
                    >
                        <span>{type.charAt(0).toUpperCase() + type.slice(1)} Charts</span>
                        {activeTab === type && <div className="h-1.5 w-1.5 rounded-full bg-accent"></div>}
                    </button>
                ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 pb-10">
                    {getChartsForTab().map((chart, idx) => (
                        <Card key={idx} className={`p-6 flex flex-col h-80 shadow-sm hover:shadow-md transition-shadow ${activeTab === 'main' && idx === 0 ? 'md:col-span-2 border-l-4 border-l-accent' : ''}`}>
                            <div className="flex justify-between items-center mb-2">
                                 <h4 className="font-medium text-slate-800 truncate" title={chart.title}>{chart.title}</h4>
                                 <Badge variant="outline">{chart.type}</Badge>
                            </div>
                            <p className="text-xs text-slate-500 mb-4 truncate">{chart.description}</p>
                    ...
                    {getChartsForTab().length === 0 && (
                        <div className="col-span-full py-12 text-center text-slate-400 border-2 border-dashed rounded-lg bg-slate-50/50">
                            No charts available for this category. The AI did not generate a {activeTab} chart for this data.
                        </div>
                    )}
                </div>
            </div>
